...
require_once DOL_DOCUMENT_ROOT.'/custom/doctemplate/core/modules/spreadsheet/vendor/autoload.php';
...
class doc_generic_odt extends ModeleThirdPartyDoc
{
	...
	function write_file($object,$outputlangs,$srctemplatepath,$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->societe->multidir_output[$object->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				$knewfileformat=substr($newfile, strrpos($newfile, '.')+1);
				
				
				
				
				if($knewfileformat == 'xlsx' || 	$knewfileformat == 'xls')
				{
					
					$newfiletmp=preg_replace('/\.xl(sx|s)/i','',$newfile);
				
				$newfiletmp=preg_replace('/template_/i','',$newfiletmp);
				$newfiletmp=preg_replace('/modele_/i','',$newfiletmp);
				}
				else if($knewfileformat == 'txt')
				{
				$newfiletmp=preg_replace('/\.tx(t)/i','',$newfile);
				$newfiletmp=preg_replace('/template_/i','',$newfiletmp);
				$newfiletmp=preg_replace('/modele_/i','',$newfiletmp);
	...
	if($newfileformat == 'xlsx' || 	$newfileformat == 'xls')
	{
				
				//create directly an object instance of the IOFactory class, and load the xlsx file
$fxls = DOL_DATA_ROOT.'/doctemplates/thirdparties/'.$filename;
...
foreach($xls_data as $xls_datavalKey=>$xls_datavalVal)
{
	foreach($xls_datavalVal as $gh=>$ty)
	{
		if (strpos($ty, '{') !== false) {
 ...
 ->setCellValue($gh.$xls_datavalKey, $py);
	}
...
$writer->save($fxls);
		...
		$this->result = array('fullpath'=>$file);
				...
				else if($newfileformat == 'txt')
				{
                    ...
                    $yu = fread($myfile,filesize($fxls));
					
                    $yuu = explode('{',$yu);
					
					$j=0;
					foreach($yuu as $yukey => $yuval)
					{
						if (strpos($yuval, '}') !== false) {
						
							 $gyuu = explode('}',$yuval);
							
							if (in_array($gyuu[0], $tmparray))
			                {
							//	if($tmparray[$gyuu[0]])
							//	{
				                 $ty = $tmparray[$gyuu[0]];
					...
					foreach($orig as $origk=>$origv)
					{
						static $gf;
						$tyu =  str_replace($orig[$origk]['ky'],$orig[$origk]['rp'],(!empty($gf)?$gf:$uio));
						$gf = $tyu;
					
					}
					
					
					$fmyfile = fopen($file, "w") or die("Unable to open file!");
                    fwrite($fmyfile, $tyu);
                    fclose($fmyfile);
