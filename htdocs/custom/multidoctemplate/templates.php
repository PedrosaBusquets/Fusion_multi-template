...
// Load translations
$langs->loadLangs(array('users', 'multidoctemplate@multidoctemplate', 'companies', 'members'));
    ...
    if (!empty($_FILES['templatefile']['name'])) {
        ...
        if (!MultiDocTemplate::isAllowedExtension($filename)) {
            ...
            if (dol_move_uploaded_file($_FILES['templatefile']['tmp_name'], $filepath, 1, 0, $_FILES['templatefile']['error']) > 0) {
                ...
                // Categories
                $custcat = GETPOST('fk_categorie_customer', 'int');
                $ctccat  = GETPOST('fk_categorie_contact', 'int');
                $template->fk_categorie_customer = $custcat > 0 ? $custcat : null;
                $template->fk_categorie_contact  = $ctccat  > 0 ? $ctccat  : null;
...
// Edit: load template to show edit form
if ($action === 'edit' && $template_id > 0 && $user->hasRight('multidoctemplate', 'template_creer')) {
    $res = $template->fetch($template_id);
    if ($res <= 0) {
        setEventMessages($template->error, null, 'errors');
        $action = '';
    }
...
// Update: save edition
if ($action === 'update' && $template_id > 0 && $user->hasRight('multidoctemplate', 'template_creer')) {
    if (!checkToken()) {
    ...
    $res = $template->fetch($template_id);
    if ($res > 0) {
        $template->label       = GETPOST('template_label', 'alphanohtml');
        $template->description = GETPOST('template_description', 'restricthtml');
        $template->tag         = GETPOST('template_tag', 'alphanohtml');
        ...
        if ($result > 0) {
            ...
            setEventMessages($template->error, null, 'errors');
            $action = 'edit'; // redisplay form with errors
        }
    } else {
        setEventMessages($template->error, null, 'errors');
        $action = '';
...
if ($user->hasRight('multidoctemplate', 'template_creer')) {
    ...
    // Customer category
    print '<tr class="oddeven">';
    print '<td>'.$langs->trans('MultiDocTemplateCustomerCategory').'</td>';
    print '<td>';
    print $formcompany->select_categories('customer', 0, 'fk_categorie_customer', 0);
    print '<br><span class="opacitymedium">'.$langs->trans('MultiDocTemplateCustomerCategoryHelp').'</span>';
    print '</td>';
    print '</tr>';

    // Contact category
    print '<tr class="oddeven">';
    print '<td>'.$langs->trans('MultiDocTemplateContactCategory').'</td>';
    print '<td>';
    print $formcompany->select_categories('contact', 0, 'fk_categorie_contact', 0);
    print '<br><span class="opacitymedium">'.$langs->trans('MultiDocTemplateContactCategoryHelp').'</span>';
    print '</td>';
    print '</tr>';
...
// Edit form
if ($action === 'edit' && $template->id > 0 && $user->hasRight('multidoctemplate', 'template_creer')) {
    print '<br>';
    print '<div class="tabsAction">';
    print '<form action="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'" method="POST">';
    print '<input type="hidden" name="token" value="'.newToken().'">';
    print '<input type="hidden" name="action" value="update">';
    print '<input type="hidden" name="template_id" value="'.$template->id.'">';

    print '<div class="fichecenter">';
    print '<table class="noborder centpercent">';
    print '<tr class="liste_titre">';
    print '<th colspan="2">'.$langs->trans('EditTemplate').'</th>';
    print '</tr>';

    // Tag (folder)
    print '<tr class="oddeven">';
    print '<td class="titlefield">'.$langs->trans('Tag').' <span class="star">*</span></td>';
    print '<td><input type="text" name="template_tag" size="40" class="flat" value="'.dol_escape_htmltag($template->tag).'" required></td>';
    print '</tr>';

    // Label
    print '<tr class="oddeven">';
    print '<td>'.$langs->trans('Label').'</td>';
    print '<td><input type="text" name="template_label" size="40" class="flat" value="'.dol_escape_htmltag($template->label).'"></td>';
    print '</tr>';

    // Description
    print '<tr class="oddeven">';
    print '<td>'.$langs->trans('Description').'</td>';
    print '<td><textarea name="template_description" rows="2" cols="40" class="flat">'.dol_escape_htmltag($template->description).'</textarea></td>';
    print '</tr>';
    ...
    print '<td>'.$langs->trans('MultiDocTemplateCustomerCategory').'</td>';
    print '<td>';
    print $formcompany->select_categories('customer', $template->fk_categorie_customer, 'fk_categorie_customer', 0);
    print '<br><span class="opacitymedium">'.$langs->trans('MultiDocTemplateCustomerCategoryHelp').'</span>';
    print '</td>';
    print '</tr>';
    ...
    print '<td>'.$langs->trans('MultiDocTemplateContactCategory').'</td>';
    print '<td>';
    print $formcompany->select_categories('contact', $template->fk_categorie_contact, 'fk_categorie_contact', 0);
    print '<br><span class="opacitymedium">'.$langs->trans('MultiDocTemplateContactCategoryHelp').'</span>';
    print '</td>';
    print '</tr>';
...
print '<th>'.$langs->trans('MultiDocTemplateCustomerCategory').'</th>';
print '<th>'.$langs->trans('MultiDocTemplateContactCategory').'</th>';
...
if (is_array($templates) && count($templates) > 0) {
    ...
    foreach ($templates as $tpl) {
        ...
        $custlabel = '';
        if (!empty($tpl->fk_categorie_customer)) {
            if ($cat->fetch($tpl->fk_categorie_customer) > 0) {
                $custlabel = $cat->label;
            }
        }
        print '<td>'.dol_escape_htmltag($custlabel).'</td>';

        // Contact category label
        $ctclabel = '';
        if (!empty($tpl->fk_categorie_contact)) {
            if ($cat->fetch($tpl->fk_categorie_contact) > 0) {
                $ctclabel = $cat->label;
            }
        }
        print '<td>'.dol_escape_htmltag($ctclabel).'</td>';

        // Actions
        print '<td class="center nowraponall">';
        if ($user->hasRight('multidoctemplate', 'template_creer')) {
            print '<a class="editfilelink" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&action=edit&template_id='.$tpl->id.'">';
    ...
    print '<tr class="oddeven"><td colspan="7" class="opacitymedium">'.$langs->trans('NoTemplatesYet').'</td></tr>';
}
